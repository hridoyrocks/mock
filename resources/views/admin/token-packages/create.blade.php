<x-admin-layout>
    <x-slot:title>Create Token Package</x-slot>

    <div class="max-w-2xl mx-auto">
        <div class="bg-white shadow-sm rounded-lg">
            <div class="p-6 border-b border-gray-200">
                <h2 class="text-xl font-semibold text-gray-900">Create New Token Package</h2>
            </div>
            
            <form action="{{ route('admin.token-packages.store') }}" method="POST" class="p-6 space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="tokens_count" id="tokens_count" value="{{ old('tokens_count', 10) }}" required min="1"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        @error('tokens_count')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="bonus_tokens" id="bonus_tokens" value="{{ old('bonus_tokens', 0) }}" min="0"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        @error('bonus_tokens')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <div class="bg-white rounded-lg p-4 border border-gray-200">
                        <div class="text-lg font-semibold" id="preview-name">Package Name</div>
                        <div class="mt-2">
                            <span class="text-2xl font-bold" id="preview-tokens">10</span>
                            <span class="text-gray-500">tokens</span>
                            <span class="text-green-600 text-sm ml-2" id="preview-bonus" style="display: none;">+0 bonus</span>
                        </div>
                        <div class="mt-2 text-xl font-bold">$<span id="preview-price">0.00</span></div>
                        <div class="mt-1 text-sm text-gray-500">
                            $<span id="preview-per-token">0.000</span> per token
                        </div>
                    </div>
                </div>
