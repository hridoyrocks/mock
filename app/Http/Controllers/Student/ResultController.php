    ...
    private function compareAnswers($studentAnswer, $correctAnswer): bool
{
    ...
    $studentAnswerClean = preg_replace('/[^\w\s]/', '', $studentAnswer);
    $correctAnswerClean = preg_replace('/[^\w\s]/', '', $correctAnswer);
    
    if ($studentAnswerClean === $correctAnswerClean) {
        return true;
    }
    ...
    $studentAnswerNorm = strtr($studentAnswerClean, $contractions);
    $correctAnswerNorm = strtr($correctAnswerClean, $contractions);
    
    if ($studentAnswerNorm === $correctAnswerNorm) {
        return true;
    }
    
    // Check for numeric equivalence (e.g., "5" vs "five")
    $numberWords = [
        'zero' => '0', 'one' => '1', 'two' => '2', 'three' => '3', 
        'four' => '4', 'five' => '5', 'six' => '6', 'seven' => '7',
        'eight' => '8', 'nine' => '9', 'ten' => '10'
    ...
    $studentAnswerNum = strtr($studentAnswerNorm, $numberWords);
    $correctAnswerNum = strtr($correctAnswerNorm, $numberWords);
    
    if ($studentAnswerNum === $correctAnswerNum) {
        return true;
    }
    
    // Reverse check (e.g., student wrote "5" but correct answer is "five")
    $reverseNumberWords = array_flip($numberWords);
    $studentAnswerWord = strtr($studentAnswerNum, $reverseNumberWords);
    $correctAnswerWord = strtr($correctAnswerNum, $reverseNumberWords);
    
    if ($studentAnswerWord === $correctAnswerWord) {
        return true;
    }
