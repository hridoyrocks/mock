...
class QuestionController extends Controller
{
    ...
    {
        // CRITICAL FIX FOR MULTIPLE CHOICE
        if ($request->question_type === 'multiple_choice') {
            // Get raw POST data
            $rawPost = $_POST;
            $correctOptions = [];
            
            // Extract correct_option values from raw POST
            foreach ($rawPost as $key => $value) {
                if ($key === 'correct_option' && is_array($value)) {
                    $correctOptions = $value;
                    break;
                }
            }
            
            // If not found, try manual extraction
            if (empty($correctOptions)) {
                $allInput = $request->all();
                foreach ($allInput as $key => $value) {
                    if (strpos($key, 'correct_option') !== false && is_array($value)) {
                        $correctOptions = $value;
                        break;
                    }
                }
            ...
            // Force set in request
            if (!empty($correctOptions)) {
                $request->merge(['correct_option' => $correctOptions]);
            }
            
            \Log::info('=== MULTIPLE CHOICE FIX ===', [
                'found_options' => $correctOptions,
                'raw_post_keys' => array_keys($rawPost),
                'request_has' => $request->has('correct_option'),
                'final_value' => $request->input('correct_option')
            ]);
        }
        
        // Enhanced Debug: Check what's coming in
        \Log::info('=== QUESTION STORE REQUEST START ===');
        \Log::info('Question Type:', ['type' => $request->question_type]);
        \Log::info('Options:', ['options' => $request->options]);
        \Log::info('Correct Option Raw:', ['correct_option' => $request->input('correct_option')]);
        ...
        switch ($section) {
            case 'listening':
                ...
                // For plan_map_diagram, no audio validation needed
                if ($request->question_type === 'plan_map_diagram') {
                    // Diagram image is handled separately
                } else {
                    // For other listening questions, check part audio
                    if (!$partAudio) {
                        return redirect()->back()
                            ->withErrors(['part_audio' => 'Please upload audio for Part ' . ($request->part_number ?? 1) . ' first.'])
                            ->withInput();
                    }
                }
        ...
        if ($this->needsOptions($request->question_type)) {
            ...
            // For multiple choice, correct_option can be array or null
            if ($request->question_type === 'multiple_choice') {
                // Don't require correct_option in validation, handle it manually
                // Allow array input for multiple choice
                $rules['correct_option'] = 'nullable|array';
                $rules['correct_option.*'] = 'nullable|string';
            } else {
                $rules['correct_option'] = 'required';
            }
        }
        ...
        // Manual validation for multiple choice correct options
        if ($request->question_type === 'multiple_choice' && $this->needsOptions($request->question_type)) {
            ...
            if (!$hasCorrectOption) {
                \Log::error('Multiple choice validation failed', [
                ...
                return redirect()->back()
                    ->withErrors(['correct_option' => 'Please select at least one correct option for multiple choice question'])
                    ->withInput();
            }
        }
        ...
        try {
            ...
            \Log::info('Starting database transaction for question creation');
            ...
            if ($section === 'listening' && $request->question_type !== 'plan_map_diagram') {
                $partAudio = $testSet->getPartAudio($request->part_number ?? 1);
                
                // Use part audio if available for non-diagram questions
                if ($partAudio) {
                    $usePartAudio = true;
                }
            }
            ...
            if (isset($typeSpecificData['diagram_hotspots'])) {
                ...
                // Set blank count for diagram questions based on number of sub-questions
                if ($request->question_type === 'plan_map_diagram') {
                    if (isset($typeSpecificData['diagram_hotspots']['sub_questions'])) {
                        $questionData['blank_count'] = count($typeSpecificData['diagram_hotspots']['sub_questions']);
                    } else {
            ...
            if ($this->needsOptions($request->question_type) && isset($request->options)) {
                // Get array of correct options (can be multiple for multiple choice)
                $correctOptions = [];
                if ($request->question_type === 'multiple_choice') {
                    // Multiple choice can have multiple correct answers
                    $correctOptionInput = $request->input('correct_option', []);
                    
                    // Debug logging
                    \Log::info('Multiple choice correct_option input:', [
                        'type' => gettype($correctOptionInput),
                        'value' => $correctOptionInput,
                        'is_array' => is_array($correctOptionInput),
                    ]);
                    
                    // Ensure it's always an array
                    if (is_array($correctOptionInput)) {
                        $correctOptions = array_filter($correctOptionInput); // Remove empty values
                    } else if (!empty($correctOptionInput)) {
                        $correctOptions = [$correctOptionInput];
                    }
                    
                    \Log::info('Processed correct options:', ['correct_options' => $correctOptions]);
                } elseif ($request->has('correct_option')) {
                    // Single correct answer for other types
                    $correctOptions = [is_array($request->correct_option) ? $request->correct_option[0] : $request->correct_option];
                }
                ...
                foreach ($request->options as $index => $option) {
                    if (!empty($option['content'])) {
                        ...
                        if ($request->question_type === 'matching_headings') {
                            ...
                            $correctOptionsStr = array_map('strval', $correctOptions);
                            $isCorrect = in_array($indexStr, $correctOptionsStr);
                            
                            \Log::info('Option comparison:', [
                                'option_index' => $index,
                                'option_index_str' => $indexStr,
                                'correct_options_str' => $correctOptionsStr,
