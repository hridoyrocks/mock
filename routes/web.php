...
Route::middleware(['web', \App\Http\Middleware\TrackReferral::class])->group(function () {
    ...
    // Maintenance page
    Route::get('/maintenance', [App\Http\Controllers\MaintenanceController::class, 'index'])->name('maintenance')->middleware('auth');
...
Route::get('/invoice/verify/{transaction}', [SubscriptionController::class, 'verifyInvoice'])->name('invoice.verify');

// Authenticated routes with role-based dashboard
Route::middleware(['auth', \App\Http\Middleware\CheckBanned::class])->group(function () {
    // Dashboard route
    Route::get('/dashboard', function () {
        if (auth()->user()->is_admin || auth()->user()->role_id) {
    ...
    Route::prefix('ai')->name('ai.')->group(function () {
        Route::post('/evaluate/writing', [AIEvaluationController::class, 'evaluateWriting'])->name('evaluation.writing')->middleware(['feature:ai_writing_evaluation']);
        Route::post('/evaluate/speaking', [AIEvaluationController::class, 'evaluateSpeaking'])->name('evaluation.speaking')->middleware(['feature:ai_speaking_evaluation']);
    ...
    Route::middleware(['admin.access'])->prefix('admin')->name('admin.')->group(function () {
        ...
        // SUBSCRIPTIONS - subscriptions permissions
        Route::middleware(['permission:subscriptions.view,subscriptions.create,subscriptions.edit,subscriptions.delete'])->group(function () {
        ...
        // COUPONS - coupons permissions
        Route::middleware(['permission:coupons.view,coupons.create,coupons.edit,coupons.delete'])->group(function () {
        ...
        // ANNOUNCEMENTS - announcements permissions
        Route::middleware(['permission:announcements.view,announcements.create,announcements.edit,announcements.delete'])->group(function () {
            Route::prefix('announcements')->name('announcements.')->group(function () {
